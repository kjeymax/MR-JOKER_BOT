...
from mrjoker import LOGGER, TEMP_DOWNLOAD_DIRECTORY
from mrjoker.utils.plh import humanbytes, progress
from mrjoker.events import register


@register(pattern=r"^/an ?(.*))", outgoing=True)
...
@register(pattern=r"^/dlup ?(.*)", outgoing=True)
...
@register(pattern=r"^/dirup ?(.*)", outgoing=True)
